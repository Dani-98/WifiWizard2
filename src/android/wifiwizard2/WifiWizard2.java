...
public class WifiWizard2 extends CordovaPlugin {
  ...
  private static final String CONNECT_WITH_IDENTITY_AND_PASSWORD = "connectWithIdentityAndPass";
  ...
  public boolean execute(String action, JSONArray data, CallbackContext callbackContext)
    ...
    if (action.equals(CONNECT_WITH_IDENTITY_AND_PASSWORD)) {
      this.connectWithIdentityAndPass(callbackContext, data);
...
//        wifiManager.reassociate();
  ...
  private void connectWithIdentityAndPass(CallbackContext callbackContext, JSONArray data) {
    Log.d(TAG, "WifiWizard2: connectWithIdentityAndPass entered.");
    ...
    try{
      ...
      Log.i("WIFI CUSTOM", "My wifi"+ wifiConfig.status);
      Integer networkId = wifiManager.addNetwork(wifiConfig);

      wifiManager.enableNetwork(networkId, true);

      WifiInfo wifiInfo = wifiManager.getConnectionInfo();
      SupplicantState supState = wifiInfo.getSupplicantState();
      Log.d("WIFI CUSTOM", supState.toString());
    } catch (Exception e){
      Log.d(TAG, "WifiWizard2: connectWithIdentityAndPass error." + e);
      return;
    }

    Toast.makeText(webView.getContext(), ssid + " " + identity + " " + password, Toast.LENGTH_SHORT).show();
  }
